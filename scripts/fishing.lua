...
function findchat()
    ...
    while #chatText < 2 do
        ...
        chatText = getChatText();
        sleepWithStatus(500,
            "Error: We must be able to read at least the last 2 lines of chat!\n\nCurrently we only see " .. #chatText ..
                " lines ...\n\nYou can also type something in main chat or manually fish, once or twice, to bypass this error!",
            nil, 0.7);
    end
...
function gui_refresh()
    ...
    y = y + 12;
    lsPrintWrapped(10, y, 0, lsScreenX - 20, 0.5, 0.5, 0xc0ffc0ff, "Current Lure: " .. CurrentLureIndex .. " of " ..
        #PlayersLures .. "   " .. CurrentLure .. " (" .. LureType .. ")");
    y = y + 12;
    ...
    y = y + 12;
    lsPrintWrapped(10, y, 0, lsScreenX - 20, 0.5, 0.5, 0xfbc3abff,
        "Last Timer: " .. round(lastCastWait / 1000, 1) .. " s");
    lsSetCamera(0, 0, lsScreenX * 1.6, lsScreenY * 1.6);

    if lsButtonText(160, y + 20, 0, 20, 0xffffffff, "-") then
    ...
    if lsButtonText(187, y + 20, 0, 20, 0xffffffff, "+") then
        QCurrentLureIndex = QCurrentLureIndex + 1
    ...
    lsPrintWrapped(115, winsize[1] - 34, 0, lsScreenX - 20, 0.5, 0.5, 0xffffffff,
        "Unusual Fish Seen: " .. GrandTotalUnusual);
    lsPrintWrapped(115, winsize[1] - 22, 0, lsScreenX - 20, 0.5, 0.5, 0xffffffff,
        "Strange Fish Seen: " .. GrandTotalStrange);
    lsPrintWrapped(10, winsize[1] - 94, 0, lsScreenX - 20, 0.5, 0.5, 0xffff40ff,
        "- - - - - - - - - - - - - - - - - - - -");
    lsPrintWrapped(10, winsize[1] - 82, 0, lsScreenX - 20, 0.5, 0.5, 0xc0ffc0ff,
        "Lures Switched: " .. GrandTotalLuresUsed - 1);
    if lastLostLure ~= "" then
        lsPrintWrapped(10, winsize[1] - 70, 0, lsScreenX - 20, 0.5, 0.5, 0xff8080ff, "Lures Lost: " ..
            GrandTotalLostLures .. "   -  Last: " .. lastLostLure .. " (" .. lastLostLureType .. ")");
    else
        lsPrintWrapped(10, winsize[1] - 70, 0, lsScreenX - 20, 0.5, 0.5, 0xff8080ff,
    ...
    lsPrintWrapped(10, winsize[1] - 22, 0, lsScreenX - 20, 0.5, 0.5, 0xffffc0ff,
        "Fish Caught: " .. GrandTotalCaught .. " (" .. math.floor(GrandTotaldb) .. "db)");
    lsSetCamera(0, 0, lsScreenX * 1.6, lsScreenY * 1.6);

    if lsButtonText(lsScreenX + 50, lsScreenY + 20, 0, 120, 0x6666FFff, "Options") then
        lsDoFrame();
        setResume = true;
        setOptions();
    end

    if lsButtonText(lsScreenX + 50, lsScreenY + 50, 0, 120, 0xFF0000ff, "End Script") then
        error(quit_message);
    end
    ...
    if not finishUp then
        if lsButtonText(lsScreenX + 50, lsScreenY + 120, 0, 120, 0xffbb80ff, "Finish Up") then
            finishUp = true;
        end
    else
        if lsButtonText(lsScreenX + 50, lsScreenY + 120, 0, 120, 0xff8080ff, "Cancel ...") then
            finishUp = false;
        end
    end

    if lsButtonText(lsScreenX + 50, lsScreenY + 150, 0, 120, skipLureTextColor, skipLureText) then
        if skipLure then
            skipLure = false;
        else
            skipLure = true;
    ...
    if lsButtonText(lsScreenX + 50, lsScreenY + 180, 0, 120, LockLureColor, LockLureText) then
    ...
    while 1 do
        ...
        elseif castcount > TotalCasts and not LockLure then
            ...
            while 1 do
                ...
                if (lastLineFound2 ~= lastLineParse2 or lastLineFound ~= lastLineParse) or
                    (OK and not ignoreOK and not skipOkOnce) or ((lsGetTimer() - startTime) > 20000) then
                    ...
                    if v == "caught" or caughtFish then
                        ...
                        if caughtFish then
                            ...
                            if SkipCommon == true and LockLure == false then
                                if FishType == "Abdju" or FishType == "Chromis" or FishType == "Catfish" or FishType ==
                                    "Carp" or FishType == "Oxyrynchus" or FishType == "Perch" or FishType == "Phagrus" or
                                    FishType == "Tilapia" then
                                    castcount = 0;
                                end
                            end
                        end
